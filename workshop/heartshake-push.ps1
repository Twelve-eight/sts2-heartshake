...
# HeartShake-specific Steam Workshop pusher (STEAMCMD-ACCESS.md flow).
# Other sessions own triple2-push.ps1 / workshop-push.ps1 - this one is only
# for the sts2-heartshake VDFs; same steamcmd core as perfect's copy.
...
& "G:\omp works\.tooling\steamcmd\steamcmd.exe" $full.Split(' ') 2>&1 |
    Tee-Object -FilePath "G:\omp works\.tmp\heartshake-push-out.txt" |
